    ...
    <a-row>
      ...
      <a-col :span="12" class="login-area">
        <a-form :model="form" :rules="rules" ref="loginForm" layout="vertical">
          ...
          <a-form-item>
            <a-button type="primary" @click="login" size="large" :loading="status.loading && status.opName === 'login'">
              {{ status.loading ? '加载中' : '登录' }}
            </a-button>
            <a-button @click="getCode" size="large" :style="{ marginLeft: '20px' }" :disabled="codeButtonDisable" :loading="status.loading && status.opName === 'getCode'">
